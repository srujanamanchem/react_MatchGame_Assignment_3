...
const initialState = {
  ...
  largeImageId: imagesList[0].id,
  largeImageUrl: imagesList[0].imageUrl,
  isGameInProcess: true,
}

const TabItem = props => {
  const {eachTab, updateActiveTabId} = props
  const {tabId, displayText} = eachTab

  const getFilteredImagesList = () => {
...
const ImageItem = props => {
  const {eachImage, checkSmallAndLargeImage} = props
  const {id, thumbnailUrl} = eachImage
  const onClickImage = () => {
    checkSmallAndLargeImage(id)
  }
  return (
...
class App extends Component {
  ...
  checkSmallAndLargeImage = imageId => {
    ...
    this.setState({
      largeImageId: imagesList[position].id,
      largeImageUrl: imagesList[position].imageUrl,
    })
  }
  ...
  render() {
    ...
    const filteredImagesList = this.getFilteredItems()
        ...
        <ul className="header-bar">
          ...
          <li className="score-timer-container">
            <p className="score-text">
              Score:
              <span className="highlight"> {gameScore}</span>
            </p>
            <li className="timer-container">
              ...
              />
              <p className="highlight"> {gameTimer} sec</p>
            </li>
          </li>
        </ul>
        <div className="game-container">
          {isGameInProcess ? (
            <div className="game-card">
              <img src={largeImageUrl} className="large-image" alt="match" />
              <ul className="tabs-list">
                {tabsList.map(eachTab => (
                  <TabItem
                    ...
                    isActiveTab={tabsList.tabId === activeTabId}
                  />
                ))}
              </ul>
              <ul className="images-list">
                {filteredImagesList.map(eachImage => (
                  <ImageItem
                    key={eachImage.id}
                    eachImage={eachImage}
                    checkSmallAndLargeImage={this.checkSmallAndLargeImage}
                  />
                ))}
              </ul>
            </div>
            ...
            <div className="game-over-card">
              ...
              <h1 className="game-end-score">{gameScore}</h1>
              <button
                type="button"
